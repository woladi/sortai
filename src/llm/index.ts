...
function applyContextualGuards(
  ...
  const bankBlock = new Set(['#KredytHipoteczny', '#KredytGotowkowy', '#LimitROR', '#BANKI', '#KartaKredytowa']);
  const preSet = new Set(req.preTags);
  const isWs48 = preSet.has('#WS48') || evidence.includes('wita stwosza') || evidence.includes('ws48');
  const isUrzad = preSet.has('#UrzadPracy');
  const isCv = preSet.has('#CV');
  const isYouth = preSet.has('#YouthRebuild') || evidence.includes('youthrebuild');
  ...
  if (isWs48) remove(t => bankBlock.has(t));
  if (isUrzad) remove(t => bankBlock.has(t) || t === '#SKD' || t === '#WakacjeKredytowe');
  if (isCv) remove(t => bankBlock.has(t));
  const strictSet = new Set(cfg.tags.strict);
  if (isYouth) remove(t => strictSet.has(t) && t !== '#RODO');
  if (noOcr) remove(t => strictSet.has(t));

  if (cleaned.includes('#BANKI')) {
    const bankKeywords = ['bank', 'pko', 'bnp', 'revolut', 'kredyt', 'wyciąg', 'prowizja'];
    if (!bankKeywords.some(k => evidence.includes(k))) {
      const idx = cleaned.indexOf('#BANKI');
